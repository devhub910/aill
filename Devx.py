...
floodtime = int(input(f"{Fore.GREEN}[>] Time: {Style.RESET_ALL}"))
thread_count = int(input(f"{Fore.GREEN}[>] Thread: {Style.RESET_ALL}"))
    ...
    while time.time() < end_time:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            try:
                ...
                print(f"{Fore.BLUE}[!] Connection failed: {e}")  # Changed to blue
